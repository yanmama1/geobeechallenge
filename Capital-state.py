...
statesinfo = dict((rows[i]["capital"], rows[i]["state"]) for i in range(0, len(rows)))
...
print "<html>"
print "<head>"
print "<title geobee state test</title>"
print "</head> <meta charset='UTF-8'>"
cap=statesinfo.values()
l=statesinfo.keys()
random.shuffle(l)
print('<form action="/cgi-bin/state-cap.py" method="post" target="_blank">')
while questions < 5:
    state=l[questions]
    v = statesinfo[state]
    answer = cap.index(v)
    ...
    qn=str(questions)
    print('Which state has the capital of ' + state +'?')
    print"<br>"
    for i in range(0,4):
